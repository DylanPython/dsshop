...
- 此分支未开发完成，请不要下载使用
- 该分支为一键安装版，当你已经安装了PHP环境后，可直接使用该版本，通过傻瓜式的环境搭建dsshop
- 该项目将默认集成所有插件，包含所有依赖，前端为已打包后的页面，无需自行打包
- 如需要二次开发，请先安装`master`和阅读开发文档后再进行二次开发，不然你将无从下手。
...
介绍
------------
Dsshop是一个商城解决方案，它采用前后端分离，后端基于laravel,前端基于Vue，我们不生成代码，我们只是代码的搬运工，该分支无需写代码即可完成项目搭建。
